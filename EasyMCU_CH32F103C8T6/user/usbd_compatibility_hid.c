...
volatile uint8_t  UART2_Tx_Flag;                                                  // UART2 TX flag
...
void UART2_DMA_Init( void )
{
    ...
    DMA_InitStructure.DMA_BufferSize = DEF_USBD_MAX_PACK_SIZE;
...
void UART2_Rx_Service( void )
{
    ...
    if (UART2_Rx_RemainLen)
    {
        ...
        if ( USBD_Endp2_Busy == 0x00)
        {
            /* Calculate the length of this packet upload via USB. */
            remain_len = UART2_Rx_RemainLen;
            pkg_len = 0x00;
            if (remain_len >= ( DEF_USBD_MAX_PACK_SIZE - 1 ))
            {
                remain_len = ( DEF_USBD_MAX_PACK_SIZE - 1 );
                pkg_len = remain_len;
            }
            else
            {
            ...
            if (pkg_len)
            {
                HIDTxBuffer[0] = pkg_len;
                memcpy(HIDTxBuffer + 1,&UART2_RxBuffer[UART2_Rx_Deal_Ptr],pkg_len);
                USBD_ENDPx_DataUp( ENDP2, HIDTxBuffer, pkg_len + 1 );
...
void UART2_Tx_Service( void )
{
    ...
    if (UART2_Tx_Flag)
    {
        if (USART_GetFlagStatus(USART2, USART_FLAG_TC))                                  // Check whether uart2 has finished sending.
        {
            ...
            UART2_Tx_Flag = 0;
            NVIC_DisableIRQ(USB_LP_CAN1_RX0_IRQn);                                       // Disable USB interrupts
    ...
    else
    {
        ...
        {
            pkg_len = Data_Buffer[(RingBuffer_Comm.DealPtr) * DEF_USBD_MAX_PACK_SIZE];      // Get the valid data length
            if (pkg_len)
            {
                if (pkg_len > ( DEF_USBD_MAX_PACK_SIZE - 1 ) )
                {
                    pkg_len = DEF_USBD_MAX_PACK_SIZE - 1;                                   // Limit the length of this transmission
                }
                pbuf = &Data_Buffer[(RingBuffer_Comm.DealPtr) * DEF_USBD_MAX_PACK_SIZE] + 1;
